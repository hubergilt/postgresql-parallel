-- ============================================================================
-- Complete Script: PostgreSQL Parallel Stored Procedures from Scratch
-- ============================================================================
...
-- Enable dblink extension (required for parallel execution)
...
BEGIN
    ...
    -- Example: Insert processing data
...
LANGUAGE plpgsql AS $$
DECLARE
    -- IMPORTANT: Update this connection string with your actual values
    -- Format: 'dbname=database_name user=username password=password host=hostname port=5432'
    conn_string TEXT := 'dbname=parallel_demo user=huber password=huber host=localhost port=5432';
    
    -- Connection names for each parallel execution
    conn_names TEXT[] := ARRAY['conn1', 'conn2', 'conn3', 'conn4', 'conn5', 'conn6', 'conn7', 'conn8'];
...
BEGIN
    ...
    -- Phase 1: Launch all procedures asynchronously
    RAISE NOTICE '';
    ...
    FOR i IN 1..8 LOOP
        BEGIN
            ...
            PERFORM dblink_connect(conn_names[i], conn_string);
            
            -- Send query asynchronously (non-blocking)
            PERFORM dblink_send_query(conn_names[i], 'CALL ' || proc_names[i] || '()');
            
            RAISE NOTICE '  [%/8] Launched % on %', i, proc_names[i], conn_names[i];
    ...
    -- Phase 2: Wait for all procedures to complete
    RAISE NOTICE 'Phase 2: Collecting results...';
    
    FOR i IN 1..8 LOOP
        BEGIN
            -- Get result (blocks until procedure completes)
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'NOTE: Each procedure runs for 3 seconds. Running sequentially';
    RAISE NOTICE '      would take 24 seconds, but in parallel should take ~3 seconds!';
    RAISE NOTICE '';
END;
$$;
